...
var keygen = cli.Command{
	Name:   "keygen",
	Usage:  "Generate a self-signed X.509 certificate for a secure connection.",
	Action: keygenAction,
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "host",
		...
		cli.StringFlag{
			Name:  "organization",
			Usage: "Organization to generate a certificate for",
		},
		cli.StringFlag{
			Name:  "start-date",
...
func keygenAction(c *cli.Context) {
	...
	default:
		fmt.Fprintf(os.Stderr, "Unrecognized elliptic curve: %q", c.String("ecdsaCurve"))
		os.Exit(1)
	}
	if err != nil {
